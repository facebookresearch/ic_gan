...
def run(config):
  ...
  for i,(x,y) in enumerate(tqdm(train_loader)):
    ...
    if i==0:
      with h5.File(config['out_path'] +
                   '/ILSVRC%i%s_xy.hdf5' % (config['resolution'],
                                            '' if not config['longtail'] else 'longtail'), 'w') \
              as f:
    ...
    # Else append to the hdf5
      ...
      with h5.File(config['out_path'] + '/ILSVRC%i%s_xy.hdf5' % (config['resolution'],
                                            '' if not config['longtail'] else 'longtail'), 'a') as f:
