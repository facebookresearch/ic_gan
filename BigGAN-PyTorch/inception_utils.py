...
from compute_pdrc import compute_prdc
...
def accumulate_inception_activations(sample, net, num_inception_images=50000):
  ...
  while (torch.cat(logits, 0).shape[0] if len(logits) else 0) < num_inception_images:
    with torch.no_grad():
      ...
      labels += [labels_val]
  return torch.cat(pool, 0), torch.cat(logits, 0), torch.cat(labels, 0) if labels[0] is not None else \
           torch.zeros(torch.cat(logits, 0).shape[0]).long()

def accumulate_features(net, loader, num_inception_images=50000,
                        device='cuda'):
  labels, pool_real = [], []
  for i, batch in enumerate(loader):
    ...
    with torch.no_grad():
      ...
      labels += [y]
    if (torch.cat(labels, 0).shape[0] if len(
            labels) else 0) >= num_inception_images:
      break

  return torch.cat(labels, 0).cpu()[:num_inception_images], torch.cat(
      pool_real, 0).cpu()[:num_inception_images]
...
def prepare_inception_metrics(dataset, samples_per_class, parallel, no_fid=False,
                              data_root='', split_name='', stratified_fid=False, prdc=False,
                              device='cuda'):
  ...
  print('Loading dataset inception moments from ',
        os.path.join(data_root, dataset + '_' +
                     'inception_moments' + split_name + '.npz'))
  stats = np.load(os.path.join(data_root, dataset + '_' +
                               'inception_moments' + split_name + '.npz'))
  data_mu = stats['mu']
  data_sigma = stats['sigma']
  if stratified_fid:
    many_stats = np.load(os.path.join(
      data_root,
      dataset + '_many_inception_moments.npz'))
    low_stats = np.load(os.path.join(data_root,
                                     dataset + '_low_inception_moments.npz'))
    few_stats = np.load(os.path.join(data_root,
                                     dataset + '_few_inception_moments.npz'))
  # Load network
  net = load_inception_net(parallel, device=device)
  def get_inception_metrics(sample, num_inception_images, num_splits=10,
                            prints=True, use_torch=True, loader_ref=None,
                              num_pr_images=10000):
    if prints:
      print('Gathering activations...')
    pool, logits, labels_val = accumulate_inception_activations(sample, net, num_inception_images)
    if prdc and loader_ref is not None:
      labels, pool_real = accumulate_features(net, loader_ref,
                                              num_inception_images,
                                              device=device)
      print('Subsampling %i samples for prdc metrics!' % (num_pr_images))
      idxs_selected = np.random.choice(range(len(pool_real)), num_pr_images,
                                       replace=False)

      prdc_metrics = compute_prdc(pool_real[idxs_selected],
                                  pool[idxs_selected].cpu(),
                                  5)
    if prints:
    ...
    if no_fid:
      ...
      FID = compute_fid(pool.clone(), data_mu, data_sigma, prints,
                        use_torch, device=device)
      stratified_fid_list = []
      if stratified_fid:
        labels_val = labels_val.cpu()
        pool = pool.cpu()
        for stats, strat_name in zip([many_stats, low_stats, few_stats],
                                     ['_many', '_low', '_few']):
          if strat_name == '_many':
            pool_ = pool[samples_per_class[labels_val] >= 100]
            print('For many-shot, selecting ', len(pool_),
                  ' samples.')
          elif strat_name == '_low':
            pool_ = pool[samples_per_class[labels_val] < 100]
            labels_ = labels_val[
              samples_per_class[labels_val] < 100]
            pool_ = pool_[samples_per_class[labels_] > 20]
            print('For low-shot, selecting ', len(pool_),
                  ' samples.')

          elif strat_name == '_few':
            pool_ = pool[samples_per_class[labels_val] <= 20]
            print('For few-shot, selecting ', len(pool_),
                  ' samples.')
            # import pdb
            # pdb.set_trace()
          stratified_fid_list.append(
            compute_fid(pool_, stats['mu'], stats['sigma'], prints,
                        False))
...
def compute_fid(pool, data_mu, data_sigma, prints, use_torch, device='cuda'):
  ...
  if use_torch:
    FID = torch_calculate_frechet_distance(mu, sigma,
                                           torch.tensor(
                                             data_mu).float().to(device),
                                           torch.tensor(
                                             data_sigma).float().to(device))
    FID = float(FID.cpu().numpy())
  else:
    FID = numpy_calculate_frechet_distance(mu,
                                           sigma, data_mu,
                                           data_sigma)
  del mu, sigma
  return FID